    ...
    for transaction in transactions:
        ...
        try:
            ...
            for payment in payments:
                if payment["to"] == ACCOUNT and payment["asset"] == "manangos":
            ...
            # Don't process a transaction with zero amount of Manangos.
            if amount <= 0:
                continue
            # Check for memo
            if not transaction["memo"]:
                LOGGER.info(f"Returning deposited Manangos, no user specified: {transaction['hash']} {transaction['source_account']} {amount}")
            ...
            try:
                ...
                LOGGER.info(f"Returning deposited Manangos, user unknown: {transaction['hash']} {transaction['source_account']} {amount}  {transaction['memo']}")
            ...
            if recipient_info is None: 
                LOGGER.info(f"Returning deposited Manangos, user has no account: {transaction['hash']} {transaction['source_account']} {amount}  {transaction['memo']}")
